...
## 화면 — 성향 설문 결과 (실서비스, 2026-09-03)

지원자 상세의 "성향 설문" 섹션. 카테고리 통계(코드 계산)와 아르의 관찰 요약(LLM)이 함께 표시되며, ADR-0027 경계에 따라 성격 유형 판정·점수화·합불 의견은 생성하지 않는다.

![성향 설문 결과 — 아르 관찰 요약 (claude-haiku)](/assets/img/aptitude-panel-haiku.png)
*아르 관찰 요약 (claude-haiku) — 상반된 응답(의견 조율 5점 vs 진행 상황 선제 공유 2점)을 관찰로 짚어내며, 문항 근거를 인용해 압축 서술한다*

![성향 설문 결과 — 로컬 sLLM 요약 (qwen3:4b)](/assets/img/aptitude-panel-qwen.png)
*같은 응답을 로컬 sLLM(qwen3:4b)이 요약한 화면 — 문항 나열식 서술로, 클라우드/로컬 모델 품질 비교의 실측 소재*
