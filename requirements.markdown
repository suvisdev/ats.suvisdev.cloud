...
기획서 밖에서 ADR로 정식 편입된 범위 — 도구 호출 에이전트(확인 카드·비용 관측), 요약 3체인, RAG 시맨틱 검색, 로컬 sLLM 이중 배포 전략, 면접 일정 자동화(FAQ 챗봇 포함), AI 면접(텍스트·STT), 인적성 설문, 제출물 무결성 원장+폴리곤 테스트넷 앵커링(ADR-0028), 면접 영상 거짓말 탐지. 각 항목의 진행 상태는 [개발 일정](/schedule/)과 [개발 로그](/devlog/)에서 확인한다.
