...
기획서 밖에서 ADR로 정식 편입된 범위 — 도구 호출 에이전트(확인 카드·비용 관측), 요약 3체인, RAG 시맨틱 검색, 로컬 sLLM 이중 배포 전략, 면접 일정 자동화, AI 면접(텍스트), 인적성 설문, **제출물 무결성 원장+폴리곤 테스트넷 앵커링(ADR-0028)** — 은 전부 구현 완료. STT 실연결은 W4 예정, 체인 발행은 운영 활성화 대기, **면접 영상 거짓말 탐지는 프로토타입 편입(09-04)** 후 서비스 연동 전.
